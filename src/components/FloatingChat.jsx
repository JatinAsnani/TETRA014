...
export default function FloatingChat() {
  ...
  function send(textToSend) {
    ...
    setTimeout(() => {
      setIsTyping(false);
      let aiReply = `Analyzing: "${text}". FRIDAy AI Engine cross-referenced your general ledger, bank feed & GSTR-3B logs. All records reconciled with zero high-risk anomalies!`;
      ...
      if (lower.includes('invoice') || lower.includes('create')) {
        ...
        aiReply = `GSTR-3B Tax Summary for August 2026: Output Tax ₹3,420.00 | Input Tax Credit (ITC) Available ₹15,120.00. Net Payable: ₹0.00 (Excess ITC ₹11,700 carried forward).`;
      } else if (lower.includes('overdue') || lower.includes('accounts')) {
        aiReply = `⚠️ 19 Overdue Accounts Flagged! Total Outstanding: ₹2,52,627.50. Top account: Anand Traders (₹99,120.00, 32 days overdue).`;
      }
